...
import { MAX_ROWS, NO_OF_BOMBS } from "../contants/contants";
...
export const generateCells = (): Cell[][] => {
    ...
    //randomly put 10 bumbs

    let bombsPlace = 0;
    while (bombsPlace < NO_OF_BOMBS) {
       const randomRow = Math.floor(Math.random() * MAX_ROWS);
       const randomCol = Math.floor(Math.random() * MAX_COLS);

       const currentCell = cells[randomRow][randomCol];
       if(currentCell.value !== CellValue.bomb){
            cells = cells.map((row, rowIndex) => row.map((
                cell, colIndex) => {
    ...
    //calculate the numbers for each cell(each cell has to have a specific number according to tbe bomb next to it);
    for(let rowIndex = 0; rowIndex < MAX_ROWS; rowIndex++){
        for (let colIndex = 0; colIndex < MAX_COLS; colIndex++){
            ...
            const bottomBomb = rowIndex < MAX_COLS - 1 ? cells[rowIndex +1][colIndex]:null;
